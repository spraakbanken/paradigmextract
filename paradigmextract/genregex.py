...
class Genregex:
    ...
    def __init__(
        self, strings: Collection[str], pvalue: float = 0.05, length: bool = True
    ):
        ...
        self.length = length
        self.stringset: Set[str] = set()
        self.prefixset = set()
        self.suffixset = set()
        self.lenrange: Tuple[int, int] = (0, 0)

        stringset: Set[str] = (
            self.strings if isinstance(self.strings, set) else set(self.strings)
        )

        # Case (1): if the totality of strings seems to have a limited distribution
        if self._significancetest(self.numstrings, len(stringset)):
            self.stringset = self.stringset
            return
        # Case (2a): find longest suffix that has limited distribution
        for i in range(-self.minlen, 0):
            suffstrings = set(map(lambda x: x[i:], stringset))
            if self._significancetest(len(list(suffstrings)), len(suffstrings)):
                self.suffixset = suffstrings
                break
        # Case (2b): find longest prefix that has limited distribution
        for i in range(self.minlen, 0, -1):
            prefstrings = set(map(lambda x: x[:i], stringset))
            if self._significancetest(len(list(prefstrings)), len(prefstrings)):
                self.prefixset = prefstrings
                break
        # Case (2c): find out if stringlengths have limited distribution
        if self.length:
            stringlengths = set(map(lambda x: len(x), stringset))
